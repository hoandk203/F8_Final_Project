...
const VendorPage = () => {
  ...
  const processOrderData = (orders: Order[]) => {
    ...
    orders.forEach(order => {
      ...
      // Kiểm tra nếu có orderDetail và là mảng
      if (order.orderDetail && Array.isArray(order.orderDetail) && order.orderDetail.length > 0) {
        // Tính tổng weight từ tất cả các orderDetails
        order.orderDetail.forEach(detail => {
          if (detail && detail.weight) {
      ...
      <div className="grid grid-cols-4 md-grid-cols-1 gap-6">
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border-l-4 border-blue-500">
            ...
            <div className="mt-2 text-sm text-gray-600 flex flex-col gap-1">
              <span>Completed: {orders.filter(o => o.status.toLowerCase() === 'completed').reduce((acc, curr) => {
                let weight = 0;
                if (curr.orderDetail && Array.isArray(curr.orderDetail)) {
                  weight = curr.orderDetail.reduce((sum, detail) => sum + (detail.weight || 0), 0);
                } else if (curr.orderDetail && curr.orderDetail.weight) {
                  weight = curr.orderDetail.weight;
                }
                return acc + weight;  
              }, 0).toLocaleString()} kg</span>
              <span>Uncompleted: {orders.filter(o => o.status.toLowerCase() !== 'completed').reduce((acc, curr) => {
                let weight = 0;
                if (curr.orderDetail && Array.isArray(curr.orderDetail)) {
                  weight = curr.orderDetail.reduce((sum, detail) => sum + (detail.weight || 0), 0);
                } else if (curr.orderDetail && curr.orderDetail.weight) {
